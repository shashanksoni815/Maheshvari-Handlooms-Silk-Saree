...
const seedData = async () => {
  try {
    ...
    const catBanarasi = (await Category.create({ name: 'Banarasi', slug: 'banarasi', description: 'Rich and heavy silk sarees from Varanasi.', isActive: true })) as any;
    const catKanjivaram = (await Category.create({ name: 'Kanjivaram', slug: 'kanjivaram', description: 'Exquisite silk from Kanchipuram.', isActive: true })) as any;
    const catChanderi = (await Category.create({ name: 'Chanderi', slug: 'chanderi', description: 'Lightweight sheer texture silk from MP.', isActive: true })) as any;
    const catMysore = (await Category.create({ name: 'Mysore Silk', slug: 'mysore-silk', description: 'Pure silk from Karnataka.', isActive: true })) as any;

    console.log('Creating Collections...');
    const colBridal = (await Collection.create({ 
